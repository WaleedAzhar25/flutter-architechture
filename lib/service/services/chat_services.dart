...
import 'package:sixam_mart/service/model/chat_message_model.dart';
import 'package:sixam_mart/service/model/contact_model.dart';
import 'package:sixam_mart/service/model/user_data_model.dart';
import 'package:sixam_mart/service/services/base_services.dart';
...
class ChatServices extends BaseService {
  ...
  Future<void> setUnReadStatusToTrue(
      {String senderId, String receiverId, String documentId}) async {
    chatServices.setUnreadCount(
        senderId: senderId, receiverId: receiverId, status: 1);
  ...
  Query chatMessagesWithPagination({String senderId, String receiverUserId}) {
    return ref
        .doc(senderId)
  ...
  Future<void> addMessageToDb(
    ...
    userRef.doc(chatData.senderId).update({
      "lastMessageTime": Timestamp.fromDate(chatData.createdAtTime.toDate())
    ...
    userRef.doc(chatData.receiverId).update({
      "lastMessageTime": Timestamp.fromDate(chatData.createdAtTime.toDate())
          .millisecondsSinceEpoch
    });
  }
  ...
  Future<void> addToSenderContacts(
    ...
    if (!senderSnapshot.exists) {
      ...
      ContactModel receiverContact = ContactModel(
          uid: receiverId,
          addedOn: currentTime,
          searchParam: receiverName.setSearchParam());
  ...
  Future<void> addToReceiverContacts(
    ...
    if (!receiverSnapshot.exists) {
      ...
      ContactModel senderContact = ContactModel(
          uid: senderId,
          addedOn: currentTime,
          searchParam: senderName.setSearchParam());
  ...
  Stream<int> getUnReadCount(
      {String senderId, String receiverId, String documentId}) {
    if (!(senderId == appStore.uid.validate())) {
      return ref
          .doc(receiverId)
          .collection(senderId)
  ...
  Future<void> clearAllMessages({String senderId, String receiverId}) async {
    final WriteBatch _batch = fireStore.batch();
  ...
  Stream<int> getUnreadNewChatCount() {
    return userRef
        .doc(appStore.uid)
        .collection(CONTACT_COLLECTION)
        .where('unReadFromUser', isEqualTo: 0)
        .snapshots()
        .map((event) => event.docs.length)
        .handleError((e) => 0);
  }

  Stream<UserData> getReceiverUserIsOnline(
      {String receiverUserId, String senderId}) {
    return userRef
        .doc(receiverUserId)
        .collection(CONTACT_COLLECTION)
        .doc(senderId)
