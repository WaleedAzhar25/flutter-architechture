...
class RazorPayServices {
  ...
  static String razorKeys;
  static BookingDetailResponse dataValue;
...
Future<void> savePay({
  ...
  Map request = {
    ...
    CommonKeys.txnId:
        txnId != '' ? txnId : "#${data.bookingDetail.id.validate()}",
  ...
  if (data.service != null && data.service.isAdvancePayment) {
    ...
    if ((data.bookingDetail.paymentStatus == null ||
            data.bookingDetail.paymentStatus !=
                SERVICE_PAYMENT_STATUS_ADVANCE_PAID ||
            data.bookingDetail.paymentStatus != SERVICE_PAYMENT_STATUS_PAID) &&
        (data.bookingDetail.paidAmount == null ||
            data.bookingDetail.paidAmount.validate() < 1)) {
      request[CommonKeys.paymentStatus] = SERVICE_PAYMENT_STATUS_ADVANCE_PAID;
    } else if (data.bookingDetail.paymentStatus ==
        SERVICE_PAYMENT_STATUS_ADVANCE_PAID) {
