...
import 'package:sixam_mart/view/customization/widget/services_widget.dart';

import '../../../data/model/response/service_provider_model.dart';

class ItemsSerivcesView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    int _length = 0;
    if (isStore) {
      _isNull = services == null;
      if (!_isNull) {
        _length = services.length;
      }
    } else {
      _isNull = items == null;
      if (!_isNull) {
          ...
          : GridView.builder(
              ...
              itemBuilder: (context, index) {
                return showTheme1Store
                    ? ServiceShimmer(isEnable: _isNull)
                    : ItemShimmer(
                        isEnabled: _isNull,
                        isStore: isStore,
